...
{
	if (input < 0) tiltAngleMax = 0;
	else	tiltAngleMax = input / 2;	// We do not need to go over 180°
	tiltAngleFull = input;
}
...
void tiltSetAngle (uint16_t input)
{
	...
	{
		tiltAngle = tiltAngleFull;
	}
	else if (input<tiltAngleMin)
	{
		tiltAngle = tiltAngleMin;
	}
	else
	{
		tiltAngle = input;
...
void buildPlatformMove (uint16_t input)
{
